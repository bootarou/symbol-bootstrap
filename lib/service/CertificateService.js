...
# the PQC catapult openssl (OPENSSLDIR=/usr/catapult/deps) has no openssl.cnf there; use the system one
...
# create CA key (ML-DSA-44 derived deterministically from the account seed)
openssl genpkey -algorithm ML-DSA-44 -pkeyopt hexseed:$(cat ca.der) -out ca.key.pem
openssl pkey -inform pem -in ca.key.pem -text -noout
openssl pkey -in ca.key.pem -pubout -out ca.pubkey.pem

${createCaCertificate}

# create node key (ML-DSA-44 derived deterministically from the account seed)
openssl genpkey -algorithm ML-DSA-44 -pkeyopt hexseed:$(cat node.der) -out node.key.pem
...
`;
    ...
    static getCertificates(stdout) {
        ...
        const extractKey = (subtext, expectedLength) => {
            const key = subtext
                .trim()
                .split(':')
                .map((m) => m.trim())
                .join('');
            if (!key || key.length !== expectedLength) {
                throw Error(`SSL Certificate key cannot be loaded from the openssl script. Output: \n${subtext}`);
            }
            return key.toUpperCase();
        };
        // ML-DSA-44: the account private key is the 32-byte seed; the public key is 1312 bytes.
        // openssl pkey -text prints the sections as: seed: ... priv: ... pub: ...
        const from = 'seed:';
        const privMarker = 'priv:';
        const middle = 'pub:';
        const to = 'Certificate';
        return locations(stdout, from).map((index) => {
            const privateKey = extractKey(stdout.substring(index + from.length, stdout.indexOf(privMarker, index)), 64);
            const pubStart = stdout.indexOf(middle, index) + middle.length;
            const publicKey = extractKey(stdout.substring(pubStart, stdout.indexOf(to, pubStart)), 2624);
            return { privateKey: privateKey, publicKey: publicKey };
        });
    }
    static createDerFile(privateKey, file) {
        // ML-DSA-44: write the raw account seed (hex) so openssl can derive the key via -pkeyopt hexseed
        (0, fs_1.writeFileSync)(file, privateKey.toLowerCase());
    }
