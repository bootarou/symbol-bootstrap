...
class AddressesService {
    ...
    async resolveAddresses(oldAddresses, oldPresetData, presetData) {
        ...
        const resolveSyncAddress = (providedAddress) => {
            ...
            addresses.sinkAddress = addresses.sinkAddress || PqcCrypto_1.PqcCrypto.generateNewAccount(networkType & 0xff).address;
        ...
        const nemesisSignerAddress = ConfigurationUtils_1.ConfigurationUtils.wrapAddress(PqcCrypto_1.PqcCrypto.addressFromPublicKey(presetData.nemesisSignerPublicKey, networkType & 0xff));
    ...
    resolveNemesisAccount(presetData, oldAddresses) {
        ...
        const signerPrivateKey = presetData.nemesis.nemesisSignerPrivateKey ||
            ...
            PqcCrypto_1.PqcCrypto.generateNewAccount(networkType & 0xff).privateKey;
    ...
    generateAddresses(networkType, privateKeySecurityMode, accounts) {
        if (typeof accounts == 'number') {
            return [...Array(accounts).keys()].map(() => ConfigurationUtils_1.ConfigurationUtils.toConfigAccount(ConfigurationUtils_1.ConfigurationUtils.toAccount(networkType, undefined, PqcCrypto_1.PqcCrypto.generateNewAccount(networkType & 0xff).privateKey)));
        }
        else {
            return accounts.map((key) => ConfigurationUtils_1.ConfigurationUtils.toConfigAccount(ConfigurationUtils_1.ConfigurationUtils.toAccount(networkType, key)));
        }
    }
