...
class ConfigService {
    ...
    async run() {
        ...
        try {
            ...
            this.preparePqcPresetData(presetData, addresses);
    ...
    // PQC: attach the nemesis signer's ML-DSA public key, a derived vrf seed, its iVRF root, and the iVRF tree depth
    preparePqcPresetData(presetData, addresses) {
        if (presetData.iVrfTreeDepth === undefined || presetData.iVrfTreeDepth === null) {
            presetData.iVrfTreeDepth = 16;
        }
        const signerPrivateKey = (addresses && addresses.nemesisSigner && addresses.nemesisSigner.privateKey) ||
            (presetData.nemesis && presetData.nemesis.nemesisSignerPrivateKey);
        if (signerPrivateKey) {
            // make the (possibly generated) nemesis signer private key available to the nemesis template
            presetData.nemesis = presetData.nemesis || {};
            presetData.nemesis.nemesisSignerPrivateKey = signerPrivateKey;
            if (!presetData.nemesisSignerVrfPrivateKey) {
                presetData.nemesisSignerVrfPrivateKey = PqcCrypto_1.PqcCrypto.deriveVrfSeed(signerPrivateKey);
            }
            presetData.nemesisSignerVrfPublicKey = PqcCrypto_1.PqcCrypto.iVrfRootFromPrivateKey(presetData.nemesisSignerVrfPrivateKey, presetData.iVrfTreeDepth);
        }
    }
    ...
    async createVrfTransaction(transactionsDirectory, presetData, node) {
        ...
        const networkByte = presetData.networkType & 0xff;
        ...
        // PQC: register the iVRF Merkle root (derived from the vrf private key), signed with ML-DSA-44
        const iVrfRoot = PqcCrypto_1.PqcCrypto.iVrfRootFromPrivateKey(node.vrf.privateKey, presetData.iVrfTreeDepth || 16);
        const payload = PqcCrypto_1.PqcCrypto.signVrfKeyLink(account.privateKey, iVrfRoot, networkByte, presetData.nemesisGenerationHashSeed);
    ...
    async createAccountKeyLinkTransaction(transactionsDirectory, presetData, node) {
        ...
        const payload = PqcCrypto_1.PqcCrypto.signAccountKeyLink(account.privateKey, node.remote.publicKey, presetData.networkType & 0xff, presetData.nemesisGenerationHashSeed);
    ...
    async createVotingKeyTransactions(transactionsDirectory, presetData, node) {
        ...
        return Promise.all(votingFiles.map(async (votingFile) => {
            const payload = PqcCrypto_1.PqcCrypto.signVotingKeyLink(account.privateKey, votingFile.publicKey, votingFile.startEpoch, votingFile.endEpoch, presetData.networkType & 0xff, presetData.nemesisGenerationHashSeed);
    ...
    async storeTransaction(transactionsDirectory, name, payload) {
        // PQC transactions are not parseable by the ed25519 symbol-sdk; write the raw payload only
