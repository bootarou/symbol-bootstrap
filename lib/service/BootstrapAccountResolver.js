...
class BootstrapAccountResolver {
    ...
    async resolveAccount(networkType, account, keyName, nodeName, operationDescription, generateErrorMessage) {
        if (!account) {
            ...
            return ConfigurationUtils_1.ConfigurationUtils.toAccount(networkType, undefined, PqcCrypto_1.PqcCrypto.generateNewAccount(networkType & 0xff).privateKey);
        }
        if (account.privateKey) {
            return ConfigurationUtils_1.ConfigurationUtils.toAccount(networkType, undefined, account.privateKey);
        }
        while (true) {
            this.logger.info('');
            this.logger.info(`${keyName} private key is required when ${operationDescription}.`);
            const address = PqcCrypto_1.PqcCrypto.addressFromPublicKey(account.publicKey, networkType & 0xff);
            ...
            if (!privateKey) {
                ...
                const enteredAccount = ConfigurationUtils_1.ConfigurationUtils.toAccount(networkType, undefined, privateKey);
